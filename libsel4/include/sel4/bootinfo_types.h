 ...
 * information chunks starting at the offset seL4_BootInfoFrameSize. Note that
 * since x86, Arm and RISC-V use 4 KiByte pages and the boot info frame usually
 * is one page, legacy userland code often contains the hard-coded assumption,
 * that this offset is 4 KiByte. Unfortunately, seL4_BootInfo currently has no
 * field that allows passing this offset. The additional boot is arch/platform
 * specific, it may or may not exist in any given execution. Each chunk has a
 * header that contains an ID to describe the chunk. All IDs share a global
