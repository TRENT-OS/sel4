                                                  ...
                                                  word_t num_bi_pages)
{
    ...
    size += num_bi_pages * BIT(seL4_BootInfoFrameBits); // boot info
                                                ...
                                                word_t num_bi_pages)
{
     ...
     * bigger.
     */
    rootserver.boot_info = alloc_rootserver_obj(seL4_BootInfoFrameBits, num_bi_pages);
                                 ...
                                 vptr_t ipcbuf_vptr, word_t num_bi_pages)
{
    ...
    for (word_t i = 0; i < num_bi_pages; i++) {
        clearMemory(
            (void *)(rootserver.boot_info + i * BIT(seL4_BootInfoFrameBits)),
            seL4_PageBits);
    }
...
)
{
    ...
    if (!provide_cap(root_cnode_cap, ut_cap)) {
        printf("Kernel init: provide_cap failed\n");
        return false;
    }
                                               ...
                                               const p_region_t *available)
{
    ...
    for (word_t i = 0; i < n_available; i++) {
        ...
        /* Sanitize the available memory region, only such memory can be used
         * that is visible in the kernel window. On 32-bit architectures this
         * is not be uncommon, because the 32-bit virtual address space is too
         * small to support a kernel windows that is big enough.
         */
        if ((r->start > PADDR_TOP) || (r->end <= PADDR_BASE)) {
            printf("    can't use region, outside of kernel window\n");
            continue;
        }
        ...
        if ((usable_reg.start != r->start) || (usable_reg.end != r->end)) {
             ...
             */
            printf("    kernel window addressable:"
