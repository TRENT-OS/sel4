...
class RISCVConfig(Config):
    ...
    def get_kernel_phys_align(self) -> int:
        '''
        Used to align the base of physical memory. Returns alignment size in
        bits, which is the same von rv32 and rv64 because rv32 just seems to
        copy what is done on rv64. This might be something to be cleaned up so
        we also use proper rv32 megapages alignment - unless this is not longer
        a hard requirement at all and 4 KiByte alignment would be sufficient
        also.
        '''
